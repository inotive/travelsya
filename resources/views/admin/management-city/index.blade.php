                    ...
                    <tbody>
                        @foreach ($cities as $key => $city)
                            <tr>
                                ...
                                <td><img width="150" src="{{ asset('storage/media/kota/'. $city->image) }}" alt="{{ $city->image }}" class="mx-auto"></td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <form id="statusForm_{{ $city->city_id }}" action="{{ route('admin.city-management.updateLanding', $city->city_id) }}" method="POST" class="me-2">
                                        @csrf
                                        @method('put')
                                        <input type="hidden" name="status" value="{{ $city->status == 0 ? 1 : 0 }}">
                                        <button id="statusButton_{{ $city->city_id }}" type="button" class="btn btn-sm {{ $city->status == 0 ? 'btn-success' : 'btn-danger' }} border-0 text-white">
                                            {{ $city->status == 0 ? 'Aktifkan' : 'Nonaktifkan' }}
                                        </button>
                                    </form>
                                    <a href="javascript:void(0)" id="btn-edit-post" data-city_id="{{ $city->city_id }}" class="btn btn-sm btn-warning border-0 text-white mb-5">EDIT</a>
                                    </div>
                                    @php
