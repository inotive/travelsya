...
class ManagementHotelController extends Controller
{
    ...
    public function settingRoomPost(Request $request)
    {
        ...
        //Ini validasi
        $request->validate(['name' => 'required', 'price' => 'required', 'sellingprice' => 'required', 'totalroom' => 'required', 'roomsize' => 'required', // 'maxextrabed' => 'required',
            'bed_type' => 'required', 'guest' => 'required',// 'image_1' => 'required|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);


        //Insetrt Data Baru
        $hotelRoom = HotelRoom::create(['hotel_id' => $request->hotel_id, 'name' => $request->name, 'price' => $request->price, 'roomsize' => $request->roomsize, 'guest' => $request->guest, 'maxextrabed' => $request->maxextrabed, 'bed_type' => $request->bed_type, 'totalroom' => $request->totalroom, 'is_active' => 1, 'sellingprice' => $request->sellingprice,]);
        ...
        foreach ($hotelRoomImageFiles as $imageFile) {
            ...
            DB::table('hotel_room_images')->insert(['hotel_id' => $hotel_id, 'hotel_room_id' => $hotelRoom->id, // Menggunakan ID dari kamar hotel yang baru saja dibuat
