    <form action="{{ route('rekreasi.order', ['id' => $package->id]) }}" method="post">
        <input type="hidden" name="price" id="package_price_{{ $package->id }}" value="{{ $today_price }}">
        @csrf
        <div class="accordion">
            <div class="card accordion-item mb-3">
                        ...
                        <div class="w-100" id="paket-{{ $key }}">
                            <button class="accordion-button collapsed bg-opacity-0 align-items-start" type="button"
                                ...
                                <div class="d-flex flex-column">
                                    <h2 class="mb-25px">{{ $package->name }} @if ($weektype == 'weekend')
                                            (Weekend)
                                        @endif
                                    </h2>
                ...
                <div class="card-body accordion-collapse collapse show" id="paket-collapse-{{ $key }}"
                    aria-labelledby="paket-{{ $key }}">
                    ...
                    <div class="mb-35px">
                        Masa Berlaku: <span class="fs-3 fw-bold">{{ \App\Helpers\General::getDateShortMonth($date) }} -
                            {{ \App\Helpers\General::getDateShortMonth(\App\Helpers\General::addingDays($date, 30)) }}</span>
                        ...
                        <div class="card-body d-flex flex-row align-items-center">
                            ...
                            <div class="d-flex justify-content-between w-100">
                                <div class="text-danger fw-bold">IDR
                                    {{ number_format($today_price, 0, ',', '.') }} / Orang </div>
                                ...
                                <div class="ms-3 input-group w-auto">
                                    ...
                                    <input type="text" class="bg-none ms-3 border-0 outline-none text-center"
                                        name="qty" id="qty_{{ $package->id }}" min="1" style="width: 32px;"
                                        value="1" readonly>
                                    <button type="button"
                ...
                <div class="card-footer">
                    <div class="d-flex flex-row align-items-center">
                        ...
                        <input type="hidden" name="weekend_ticket" value="{{ $weektype }}">
