        ...
        <div class="modal-content">
            ...
            <div class="modal-body bg-snow-pink min-h-350">
                ...
                <div class="p-5 bg-snow-pink">
                    <form action="{{ route('bus_travel.order') }}" method="post" class="w-100">
                    @csrf
                    ...
                    @if (isset($is_order))
                        @for ($i = 1; $i <= $jumlah_penumpang; $i++)
                        <input type="hidden" name="kursi_penumpang_{{ $i }}" id="kursi_pilihan_penumpang_{{ $i }}" value="{{ ${'kursi_penumpang_'.$i} }}">
                        @endfor
                    @else
                        @for ($i = 1; $i <= $jumlah_penumpang; $i++)
